...
const (
	insert         = "INSERT INTO companies (id, name, address, description) VALUES ($1, $2, $3, $4)"
	get            = "SELECT u.email, c.id, c.name, c.address, c.description, c.approved FROM companies c INNER JOIN users u ON u.id = c.id WHERE c.id = $1"
	getApproved    = get + " AND c.approved = true"
	getAll         = "SELECT u.email, c.id, c.name, c.address, c.description, c.approved FROM companies c INNER JOIN users u ON u.id = c.id ORDER BY c.created_at DESC"
	getAllApproved = "SELECT u.email, c.id, c.name, c.address, c.description, c.approved FROM companies c INNER JOIN users u ON u.id = c.id WHERE c.approved = TRUE ORDER BY c.created_at DESC"

	update         = "UPDATE companies <set> WHERE id = $1"
	setName        = "name = ?"
	setAddress     = "address = ?"
...
func (p *PgRepo) Get(ctx context.Context, id string, onlyApproved bool) (*models.CompanyInfo, error) {
	q := get
	if onlyApproved {
		q = getApproved
	}
...
func (p *PgRepo) Update(ctx context.Context, id string, cu *models.CompanyUpdate) error {
	if cu.IsEmpty() {
